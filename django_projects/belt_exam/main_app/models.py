...
class User(models.Model):
    ...
    # shows = LIST OF ALL SHOWS CREATED BY THIS USER
...
class ShowManager(models.Manager):
    def validate_show(self, postData):
        # 1. CREATE EMPTY ERRORS DICTIONARY
        errors = {}
        
        # 2. VALIDATE AND FILL OUT ERRORS MESSAGES
        # TITLE MUST BE 2 CHARS
        if len(postData['title']) < 2:
            errors['title'] = "TITLE MUST BE AT LEAST 2 CHARS"

        # NETWORK MUST BE 3 CHARS
        if len(postData['network']) < 3:
            errors['network'] = "NETWORK MUST BE AT LEAST 3 CHARS"

        # RELEASE DATE MUST BE PROVIDED
        if len(postData['release_date']) == 0:
            errors['release_date'] = "RELEASE DATE MUST BE PROVIDED"

        # DESCRIPTION MUST BE 10 CHARS
        if len(postData['description']) < 10:
            errors['description'] = "DESCRIPTION MUST BE AT LEAST 10 CHARS"
        
        # 3. RETURN THE ERRORS DICTIONARY
        return errors
